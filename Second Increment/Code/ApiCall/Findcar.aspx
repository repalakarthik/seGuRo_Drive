<%@ Page Language="C#" MasterPageFile="~/SiteHTML5.master"  AutoEventWireup="true" CodeBehind="Findcar.aspx.cs" Inherits="ApiCall.Findcar" %>
...
<asp:Content ID="BodyContent" runat="server" ContentPlaceHolderID="MainContent">
    <label> By default the Current location is set as the Car Parking Location. 
    Please enter the Latitude and Longitude of any other location and hit Find Car, which calculates the routes and display</label>
     <label> Please Enter the Latitude and longitude</label>
         <br/>
      <label>
          Latitude
   ...
   <%-- <asp:Button id="carLocation"  
           ...
           runat="server" Text="Parking Location" onclientclick="getLocation()" />--%> <%-- onclick="btncarLocation_Click"--%>
           <%--<input type="button"  id="carLocation" title="Parking Location" style=" background-color:Transparent;padding-right:5px;margin:0px;  color:#0000CD;font-weight:bold; font-size:10pt;cursor:pointer; float:inherit" value="Parking Location" />         --%>
          <input type="button"  id="findCar" title="find car" style=" background-color:Transparent;padding-right:5px;margin:0px;  color:#0000CD;font-weight:bold; font-size:10pt;cursor:pointer; float:inherit" value="Find Car" />         
...
<%--
  if (navigator.geolocation) { //Checks if browser supports geolocation
                navigator.geolocation.getCurrentPosition(function(position) { //This gets the
                    ...
                    var map = new google.maps.Map(document.getElementById("map-canvas"), mapOptions /*Creates a new map using the passed optional parameters in the mapOptions parameter.*/);
                    directionsDisplay.setMap(map);
